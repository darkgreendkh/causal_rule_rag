...
默认 RPC 阈值为零点七，用于拒绝明显缺乏结构闭合或时序相容的对象，但达到阈值不等于通过专家审查。它是尚未实验调优的工程参数，后续须同时观察错误关系保留与正确关系误删，不能只报告生成边数。

本文在第6章对该阈值做了生效检查，结果是否定的：当前规则包构建出的 2074 条方向边的 RPC 取值集合为 {1.0}，无一条低于阈值，去除 RPC 筛选后构图结果与完整配置逐项相同（见表6-8）。原因在于参与构图的规则均已由实现者对照原文核对，覆盖、衔接与时序三个分量同时取满，RPC 因而退化为二值标志：不满足结构要求的候选在更早的表达式校验阶段即被拒绝，进入评分的候选一律得满分。因此本文不主张 RPC 筛选改善了构图质量，只主张它以可追溯的形式记录了结构一致性状态。要使该机制可被检验，需要让未经人工核对的模型候选进入同一评分通道，并按候选分布重新标定阈值。
...
与 RPC 相同，该阈值在本语料上也未产生筛选作用：521 条跨层边的 SCS 最小值为 0.9226，全部高于阈值（见表6-8）。这一结果可以从式(3-2)的输入直接解释——语义分量比较的是规则标签与规则自身引用原文的稠密表示，两段文本天然高度重合，因而 \(S_{ur}\) 普遍接近上界。结论与 3.5.3 一致：SCS 当前承担的是记录跨层支持依据的职责，其筛选能力需要在引入未经核对的候选后重新标定并单独验证，本文不把它列为已验证的有效组件。
