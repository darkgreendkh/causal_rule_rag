...
第6章在未收缩的原图上对展开后的归属做了统一评价（表6-9），结果显示收缩带来的是确定性而非质量的全面提升。收缩使全部 2074 条强依赖边同群，五个随机种子间的归一化互信息由 Leiden 的 0.889 升至 0.927；代价是模块度由 0.7788 降至 0.7666，最大社区占比由 8.7% 升至 13.7%，事项纯度由 0.586 降至 0.320。需要同时指出，无约束的 Leiden 在本图上已经保持了 99.9% 的强依赖边，仅两条被切断。因此收缩的价值应表述为：把“强依赖通常不被切断”这一经验性质变为构造上的保证，并降低划分对随机种子的敏感性；而不应表述为显著改善了社区质量。当依赖链跨越多个事项时，收缩会把这些事项并入同一社区，主题一致性随之下降，这是该设计必须承担的代价。
...
平均相似度降低了单个偶然高分节点独占社区排序的可能性，同时也存在明确代价。一个社区如果包含大量引用相同来源的派生节点，均值会受到该来源的重复表示影响；大型社区中少数高度相关规则也可能被低分成员稀释。这些是当前聚合方式的可验证偏差来源。

第6章的检索实验（表6-10）证实了这一代价，并划出了本方法的适用边界。当查询绑定事项时，候选节点已被事项过滤，粗筛只在小范围内排序，有向约束检索的 nDCG@8 由向量检索的 0.639 提升至 0.816。当查询不绑定事项时，粗筛需要在全图 51 个顶层社区上按式(4-4)选出前三个，一旦目标条款所属社区未被选中，后续有向搜索便无法访问该条款；此时召回由 0.629 降至 0.454，证据完整率由 0.442 降至 0.357，低于纯向量检索。精确率仍高于基线，说明被检出内容的质量较好，但覆盖面明显收窄。因此本文不主张社区粗筛普遍提高召回，而限定为：在查询已绑定事项时，粗筛以很小的召回代价换取显著的排序与精确率改善；在开放查询上应放宽粗筛数量或回退到向量候选。该取舍在 7.2 中作为限制保留。
...
第6章的消融实验（表6-11）对这三项给出了不一致的结果，其中语义项的方向与预期相反。移除式(4-8)中的语义项后，证据召回由 0.819 升至 0.875，证据完整率由 0.675 升至 0.812。原因可以从 \(S(P,q)\) 的定义看出：它取路径上全部节点相对问题向量的相似度平均，路径越长越容易被中间过渡节点拉低，权重 0.4 的语义项因而系统性地压低长路径得分；而补齐一个合取条件所需的证据往往正分布在较长的依赖链上。RPC 项在本语料上对所有候选贡献同一常量（见 3.5.3），因此不改变相对次序；熵项影响很小。据此，本文不主张式(4-8)的权重配置已经过验证；语义分量的合理修正方向是按路径长度归一化或只对起止节点计分，该修改需重新验证后才能进入结论，本文不在测试集上反复调参后回填有利配置。
