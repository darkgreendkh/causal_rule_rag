...
在实验方面，本文按金标准来源把验证分为两层，并完成了第一层的七组运行。结果对三条主线给出了不同强度的支持。知识表示方面，去掉任一图层都会使可追溯性或可执行性下降，双层结构的必要性得到结构数据支持；但RPC与SCS两个一致性阈值在本语料上从未拒绝任何候选，因而不能作为有效性主张。检索方面，依赖约束使强依赖边保持率达到100%并提高划分稳定性，其代价是模块度与事项纯度下降；有向约束检索只在查询绑定事项时优于向量检索，开放查询上召回反而下降；路径评分中的语义分量在证据完整率上是负作用。流程计算方面结论最明确：在283个注入缺陷上，受约束序列修复全部返回通过独立复重放的合法计划，平均单位编辑代价1.000且不超过注入代价上界，优于截断贪心与重新规划两个基线；来源移除后的六个重建场景没有残留过期引用。

需要同时说明两点。其一，第一层实验的检索评测使用的是与图谱构建同源的银标准，其精确率优势不构成独立效果证据；条款抽取与问答质量必须由第二层的独立人工标注回答，在其完成之前本文不对回答正确性与规范解释质量作结论。其二，面向任务的自动评测发现了工程回归测试未覆盖的实现缺陷——下一步推荐曾把已完成动作计入可执行候选——说明单元测试与任务级评测覆盖不同风险，二者都不可省略。
...
还有三项由实验直接暴露的设计限制。第一，一致性筛选未生效：RPC取证据覆盖、表达式衔接与时间顺序三个分量的等权平均，而参与构图的规则均已通过实现者原文核对，三个分量同时取满，使RPC退化为二值标志；SCS的语义分量取规则标签与其自身引用原文的相似度，两段文本本就高度重合。要使该机制可检验，必须让未经核对的模型候选进入同一评分通道并按候选分布重新标定阈值。第二，检索方法依赖事项情境：社区粗筛在开放查询下按全图节点得分均值选社区，一旦目标条款所属社区未入选，后续有向搜索无法补救，召回损失不可由排序弥补。第三，路径评分的语义分量按节点平均计算，对较长路径产生系统性惩罚，而完整AND条件所需的证据往往分布在较长依赖链上。三项限制的修正方向分别是重新标定阈值、在开放查询上回退向量候选、按长度归一化语义分量，均需重新验证后才能写入结论。

最后，系统尚未完成需要人工标注的评测。问答忠实性、证据充分性、规则抽取质量与修复方案的业务可接受性需要不同的标注与评价方法，第一层的自动指标不能替代。若只关注答案流畅程度，可能忽略来源失效或遗漏必要条件；若只关注程序通过率，又可能忽略规则包本身的解释缺陷。因此，后续评价需要把资料、规则、检索、生成和交互分别纳入分析。
