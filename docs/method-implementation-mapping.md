...
本文按当前代码记录可追溯的实现，不把设计名词当作效果结论。此处的验证用例是工程检查，与实验分开；金标准可程序化确定的第一层实验（E04—E07、E09—E11）已在 `backend/experiments/` 实现并运行，需要人工标注的第二层（E01—E03、E08）尚未开展，见 `docs/experiment-plan.md`。真实全量 HTTP、浏览器验收由主集成任务执行。
...
7. 本文只报告单元测试、集成测试、清点数量和研发运行记录。效果数值来自 `docs/thesis/experiments/` 的第一层实验，不在本文重复；仍无人工标注结果、用户研究或显著性检验。第一层已否定两项设计预期：RPC/SCS 阈值在本语料未生效，路径语义评分项降低证据完整率。
