...
资格规则绑定实际受限动作；`purpose=condition_check` 同时支持空步骤的事实核查。准备动作不提前执行提交阶段资格条件。修复只对拟执行序列做单位代价的插入、删除、替换，保留历史、政策参数与不可变事实；达到状态预算返回 truncated，不宣称全局无解。下一步只返回尚未完成动作的可预览结果，不持久化真实办理状态。
