...
- React 页面展示文档进度、原始分块、实体关系、答案来源和图谱路径。
...
3. 点击“查看分块”，检查法规条款和 Markdown 表格是否保持完整。
...
| `POST` | `/api/qa` | `{question, mode: "vector" | "hybrid"}` |
...
会话记忆、reranker、关键词检索、流式输出、图谱人工编辑和生产部署。后续创新点应以
独立实验变量逐项接入，保留 `vector` 模式作为消融对照。
